...
- **Language Model Data**: `reviewed` and `experimental-full` carry bigram and trigram statistics from the TRAIN partition of the Kurmancî Wikipedia corpus, and of that corpus only, for next-word prediction (`data/language-model/kuwiki-20260801/`: numeric tables over vocabulary ids, no corpus text, restricted to the pack vocabulary and pinned by SHA-256 to the exact dump, partition, and corpus-scoped TRAIN-only statistics; a model built for an older pack vocabulary fails closed). Unigram frequencies are committed alongside for frequency-aware ranking experiments (`model_profile = "frequency"` / `"ngram"`). Statistics never add or approve words; corpus attribution and a pending redistribution determination are recorded in every model-backed pack.
