...
| **Controlled Review Infrastructure (4A.1)** | Active | Length-prefixed SHA-256 canonical entry/group identity, rule-driven review queue generator (`review-queue-v1`), decision store validator (`review-decision-v1`), and merged audit reporting (`controlled-review-report-v1`). |
| **Controlled Pack Policy & Builds (4A.2)** | Planned | `pack-policy.toml` configuration, `seed`, `reviewed`, and `experimental-full` multi-pack builds, model-profile separation, pack manifests, and licensing attribution. |
...
| **Human-Reviewed Benchmarks & Model Comparison (3C3)** | Planned | Expanded human-reviewed spelling and context benchmarks, unigram vs bigram vs trigram hard-backoff comparison engine. |
