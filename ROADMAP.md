...
| **Controlled Review Infrastructure (4A.1)** | Completed | Length-prefixed SHA-256 canonical entry/group identity, rule-driven review queue generator (`review-queue-v1`), decision store validator (`review-decision-v1`), and merged audit reporting (`controlled-review-report-v1`). |
| **Controlled Pack Policy & Builds (4A.2)** | Completed | `pack-policy.toml` configuration, `seed`, `reviewed`, and `experimental-full` multi-pack builds, model-profile separation, pack manifests, and licensing attribution. |
| **Human-Reviewed Evaluation Dataset & Pack Comparison (4B)** | Active | Human-reviewed benchmark dataset (`draft-cases.jsonl` vs `reviewed-cases.jsonl`), three-pack comparison engine (`seed` vs `reviewed` vs `experimental-full`), false-acceptance and regression reporting. |
